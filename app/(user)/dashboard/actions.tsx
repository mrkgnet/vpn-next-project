...
export async function addUserAction(formData: FormData) {
    ...
    try {
        ...
        if (existingClient) return { success: false, message: "این کاربر از قبل وجود دارد، لطفا از بخش آپدیت استفاده کنید." };
...
export async function updateUserAction(formData: FormData) {
    console.log("\n🚀 --- شروع آپدیت (روش جایگزین: Update Inbound) ---");
    ...
    try {
        ...
        for (const inbound of inbounds) {
            ...
            if (typeof inbound.settings === 'string') {
                try { inbound.settings = JSON.parse(inbound.settings); } catch (e) {}
            }
            // پارس کردن تنظیمات استریم (مهم)
            if (typeof inbound.streamSettings === 'string') {
                try { inbound.streamSettings = JSON.parse(inbound.streamSettings); } catch (e) {}
            }
            // پارس کردن تنظیمات اسنیفینگ
            if (typeof inbound.sniffing === 'string') {
                try { inbound.sniffing = JSON.parse(inbound.sniffing); } catch (e) {}
            }
        ...
        if (result.success) {
            revalidatePath('/products');
            return { success: true, message: `کاربر ${cleanEmail} با موفقیت تمدید شد` };
        } else {
